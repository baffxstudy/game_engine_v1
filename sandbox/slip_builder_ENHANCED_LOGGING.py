...
class MarketRegistry:
    ...
    def build(self, payload: Dict[str, Any]) -> None:
        ...
        if matches and len(matches) > 0:
            ...
            logger.info(f"[REGISTRY] First match keys: {list(first_match.keys())}")
            logger.info(f"[REGISTRY] First match ID: {first_match.get('match_id', first_match.get('id', 'NO_ID'))}")
            if 'match_markets' in first_match:
                markets_sample = first_match['match_markets']
                logger.info(f"[REGISTRY] First match has 'match_markets' with {len(markets_sample)} items")
                if markets_sample and len(markets_sample) > 0:
                    first_market = markets_sample[0]
                    logger.info(f"[REGISTRY] First market keys: {list(first_market.keys())}")
                    logger.info(f"[REGISTRY] First market type: {first_market.get('market_type', 'NO_TYPE')}")
                    logger.info(f"[REGISTRY] First market selections count: {len(first_market.get('selections', []))}")
            else:
                logger.warning(f"[REGISTRY] First match does NOT have 'match_markets' key!")
    ...
    def _parse_markets_flexible(self, match_id: int, markets_data: List[Dict]) -> Dict[str, List[MarketSelection]]:
        ...
        for market_idx, market_entry in enumerate(markets_data):
            ...
            # Structure B: {"market_type": "MATCH_RESULT", "outcomes": [...]}
                ...
                logger.info(f"Match {match_id}: Structure B (Laravel) detected, market_type={market_code}")
                if not market_code:
                    logger.warning(f"Match {match_id}: market_type key exists but value is empty/null!")
            ...
            if "selections" in market_entry:
                ...
                logger.info(f"Match {match_id}, market {market_code}: Found {len(selections_data) if isinstance(selections_data, list) else 'NON-LIST'} selections under 'selections' key")
    ...
    def _parse_selections_flexible(self, match_id: int, market_code: str, selections_data: List[Dict]) -> List[MarketSelection]:
        ...
        for sel_idx, selection_data in enumerate(selections_data):
            ...
            for key in possible_value_keys:
                if key in selection_data:
                    ...
                    if candidate is not None and str(candidate).strip():
                        ...
                        logger.info(f"Match {match_id}, market {market_code}, sel {sel_idx}: found value '{value}' under key '{key}'")
                        break
            ...
            for key in possible_odds_keys:
                if key in selection_data:
                    ...
                    try:
                        ...
                        logger.info(f"Match {match_id}, market {market_code}, sel {sel_idx}: found odds {odds} (raw: '{odds_raw}') under key '{key}'")
                        break
                    except (TypeError, ValueError, ArithmeticError) as e:
                        logger.warning(f"Match {match_id}, market {market_code}, sel {sel_idx}: odds under '{key}' invalid: {odds_raw}, error: {e}")
                        continue
            ...
            try:
                ...
                logger.info(f"Match {match_id}, market {market_code}: ✓ ACCEPTED selection '{value}' @ {odds}")
